...
// // 💡Initial Load Note: The HTML is set up by default for dark mode (no 'light-mode' class on body), so we need to set the initial toggle state to checked (ON) as dark is the default. You might need to adjust the HTML or call applySavedTheme() to ensure the toggle is ON when the page loads to dark mode. The initial state is handled by the function above.
...
function updateWeatherUI(weatherData) {
  ...
  const formattedTime = localTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
  const formattedDate = localTime.toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long' });
...
// ---------------------- UPDATE FORECAST UI ----------------------
function updateForecastUI(forecastData) {
  const hourlyData = forecastData.list.slice(0, 6);
  hourlyData.forEach((val, index) => {
    const hourTemp = Math.floor(val.main.temp);
    const hourwind = val.wind.speed;
    const timestamp = val.dt * 1000;
